...
class FlowClient:
    ...
    async def ensure_token(self):
        ...
        if not token:
            ...
            raise RuntimeError("Could not get Google session access token. Please check login state.")
    ...
    async def _generate_video_whisk(self, prompt, image_paths=None, model="veo-3.1-fast", aspect_ratio="16:9", duration=8, project_name="", count=1, callback=None):
        ...
        for nav_attempt in range(max_nav_retries):
            ...
            try:
                await self._page.goto("https://flow.google/", wait_until="load", timeout=60000)
                await asyncio.sleep(2)
            except Exception as e:
            ...
            try:
                welcome_dialog = await self._page.query_selector('md-dialog[open]')
                if welcome_dialog:
                    got_it = await welcome_dialog.query_selector('button:has-text("Got it"), button:has-text("Đã hiểu")')
                    if got_it:
                        await got_it.click()
                    else:
                        continue_btn = await self._page.query_selector('button:has-text("Continue")')
                        if continue_btn:
        ...
        try:
            ...
            async def click_segment_button(options):
                def clean_text(text):
                    import re
                    return re.sub(r'[^a-z0-9]', '', str(text).lower())
                
                # 1. Try get_by_text first (most reliable for spans/labels)
                for opt in options:
                    if len(opt) < 2: continue
                    try:
                        locs = await self._page.get_by_text(opt, exact=False).all()
                        for loc in locs:
                            if await loc.is_visible():
                                box = await loc.bounding_box()
                                if box and box['y'] > 50 and box['width'] < 300 and box['height'] < 100:
                                    # Use mouse click to perfectly simulate human interaction and bypass React pointer event blocking
                                    cx = box['x'] + box['width'] / 2
                                    cy = box['y'] + box['height'] / 2
                                    await self._page.mouse.click(cx, cy)
                                    
                                    # Smart Dropdown Handler: Wait to see if a listbox appeared
                                    await asyncio.sleep(0.5)
                                    listbox = await self._page.query_selector('[role="listbox"], md-menu, .mat-mdc-select-panel')
                                    if listbox and await listbox.is_visible():
                                        log.info("UI Auto: A dropdown listbox appeared! Selecting option inside...")
                                        opts = await listbox.query_selector_all('md-select-option, [role="option"], md-menu-item, mat-option')
                                        clicked_opt = False
                                        for opt_el in opts:
                                            text_el = await opt_el.inner_text()
                                            c_text = clean_text(text_el)
                                            for opt_val in options:
                                                if clean_text(opt_val) in c_text and len(c_text) > 0:
                                                    await opt_el.click(force=True)
                                                    clicked_opt = True
                                                    break
                                            if clicked_opt: break
                                        if not clicked_opt:
                                            await self._page.keyboard.press("Escape")
                                    return True
                    except Exception as e:
                        log.warning(f"Error in get_by_text: {e}")
                        pass
                
                # 2. Fallback to query_selector_all (added label and div)
                buttons = await self._page.query_selector_all('button, label, div[role="button"], [role="radio"], [role="tab"], md-filter-chip')
                for b in buttons:
                    if await b.is_visible():
                        ...
                        # Ignore massive paragraphs like sample cards!
                        if len(text) > 50:
                            continue
                            
                        for opt in options:
                            opt_lower = opt.lower().strip()
                            # Require exact match for very short options (like "1", "1x")
                            if len(opt_lower) <= 2:
                                match = (opt_lower == text)
                            else:
                                match = (opt_lower in text or opt_lower in aria)
                                
                            if match:
                                # extra safety for 'veo'
                                if opt_lower == "veo" and "veo" not in text.split():
                                    continue
            ...
            try:
                search_texts = ["Veo", "Imagen", "Omni", "Nano", "Banana", "Flash", "Video", "Image", "16:9", "1:1", "4:3", "9:16", "3:4", "8s", "4s", "x4", "x1", "x2", "x3", "1x", "2x"]
                for text in search_texts:
                    ...
                    for loc in locs:
                        if await loc.is_visible():
                            box = await loc.bounding_box()
                            if box:
                                # Ensure we click the interactive container
                                btn_handle = await loc.evaluate_handle('el => el.closest("button, [role=\'button\'], md-suggestion-chip, div") || el')
            ...
            if settings_btn:
                try:
                    ...
                    try:
                        ...
                        await settings_btn.evaluate("el => el.click()")
                await asyncio.sleep(2.0)
            else:
                log.warning("UI Auto: CRITICAL - Could not find Settings Pill button!")

            # Switch to Video tab (Safeguarded: Find by text + location instead of rigid roles)
            try:
                await asyncio.sleep(1)
                
                # 1. First try to find elements with "Video" text
                video_locs = await self._page.get_by_text("Video", exact=False).all()
                for loc in video_locs:
                    if await loc.is_visible():
                        box = await loc.bounding_box()
                        # ONLY accept elements in the bottom half of the screen (Settings popup) and right side (not sidebar)
                        if box and box['y'] > 300 and box['x'] > 300:
                            log.info("UI Auto: Switching to Video mode tab in popup (via get_by_text and mouse click)...")
                            # Use absolute mouse click to completely bypass DOM event listener issues!
                            cx = box['x'] + box['width'] / 2
                            cy = box['y'] + box['height'] / 2
                            await self._page.mouse.click(cx, cy)
                            await asyncio.sleep(1)
                ...
                log.warning(f"UI Auto: Error switching to Video tab: {e}")

            # 3. Model (Dropdown or Segment)
            if model:
                log.info(f"UI Auto: Selecting model {model}...")
                # We now rely on clean_text matching, so just passing the model name directly is safe
                # We remove "Quality", "Fast", "Lite" from "Veo 3.1" extensions since we want to match EXACTLY
                model_options = [model]
                base_model = model.split('-')[0].strip()
                if base_model != model:
                    model_options.append(base_model)
                
                if "veo" in model.lower():
                    model_options.append("Veo")
                # If model is Omni Flash, maybe it's just Omni in UI
                if "omni flash" in model.lower():
                    model_options.append("Omni")
                    
                # Try to find the model dropdown. Restrict to elements inside the popup (y < 600) to avoid clicking the Settings Pill again.
                known_model_keywords = ["veo", "omni", "imagen", "nano"]
                triggers = await self._page.query_selector_all('button, div[role="button"], md-select, md-outlined-select, [role="combobox"], [aria-haspopup="listbox"]')
                dropdown_opened = False
                for trigger in triggers:
                    if await trigger.is_visible():
                        rect = await trigger.bounding_box()
                        if not rect or rect['y'] > 600:
                            continue  # Skip anything at the bottom of the screen (e.g., the Settings Pill)
                            
                        text = (await trigger.inner_text() or "").lower()
                        aria = (await trigger.get_attribute("aria-label") or "").lower()
                        combined = f"{text} {aria}"
                        if any(k in combined for k in known_model_keywords) and len(combined) < 60:
                            log.info(f"UI Auto: Clicking likely model dropdown trigger...")
                ...
                if not clicked:
                    log.info("UI Auto: Dropdown failed or not present, trying segment buttons for model...")
            # 1. Ratio (Segment buttons or Dropdown handled robustly)
            ...
            # Capture existing videos BEFORE doing any UI manipulation that could trigger auto-generation
            ...
            # 1. Find the prompt input by scanning all text editors and picking the one at the absolute bottom
            prompt_input = None
            max_y = -1
            ...
            await asyncio.sleep(0.5)
            try:
                gen_btn = await self._page.query_selector('button[aria-label*="Generate"], button[title*="Generate"], button:has-text("Generate"), button[aria-label*="Tạo"], button[title*="Tạo"], button:has-text("Tạo")')
                if gen_btn and await gen_btn.is_visible():
            ...
            await self._page.evaluate('''() => {
                ...
                const observer = new MutationObserver((mutations) => {
                    ...
                    const dbtn = document.querySelector('button[aria-label*="ownload"], button[title*="ownload"], [aria-label*="ownload"]');
            ...
            while (_time.time() - start_time) < max_wait:
                ...
                if result_imgs:
                    dump_info = await self._page.evaluate('''() => {
                        return Array.from(document.querySelectorAll('video')).map(v => v.src || v.currentSrc || "no_src");
                    }''')
                    log.info(f"EVIDENCE: Found {len(result_imgs)} video tags in DOM.")
                    log.info(f"EVIDENCE: Video SRCs: {dump_info}")
                else:
                    log.info(f"EVIDENCE: No video tags in DOM yet.")
                # --------------------------------------------------------
                if result_imgs:
                    v_data = await self._page.evaluate(
                        '''(els) => els.map(v => {
                            const rect = v.getBoundingClientRect();
                            // Handle both src and currentSrc (for <video> with <source> children)
                            const actualSrc = v.currentSrc || v.src || "";
                            return {
                                src: actualSrc, 
                                visible: (rect.width > 0 || v.readyState > 0 || actualSrc.includes('blob:'))
                            };
                        })''', result_imgs
                    )
                    new_srcs = []
                ...
                for src in new_srcs:
                    log.info(f"UI Auto: Processing new video src: {src[:100]}...")
                    if "download_started" not in perf_timestamps: perf_timestamps["download_started"] = perf_time.perf_counter()
                    
                    # Fetch blob/video size via browser (works for both blob: and http: URLs to bypass 403)
                    blob_info = await self._page.evaluate(
                        '''async (url) => {
                            try {
                                ...
                                // If fetch fails (CORS), but it's an HTTPS URL, just return the URL!
                                if (url.startsWith('https://')) {
                                    return {size: 100000, data: url, error: "CORS fallback"};
                                }
                                return {size: 0, data: null, error: e.message || "Unknown error"}; 
                            }
                        }''', src
                    )
                    ...
                    if blob_info and blob_info.get("data"):
                        ...
                        log.warning(f"UI Auto: Failed to fetch video {src[:50]}: {err}. Attempting Download button fallback...")
                        
                        # Fallback: Find and click the Download button on the UI
                        try:
                            # Try to hover over the video to reveal hidden buttons
                            videos = await self._page.query_selector_all('video')
                            if videos:
                                try:
                                    await videos[-1].hover(force=True, timeout=1000)
                                    await asyncio.sleep(0.5)
                                except Exception:
                                    pass

                            dbtns = await self._page.query_selector_all('button[aria-label*="ownload"], button[title*="ownload"], [aria-label*="ownload"]')
                            # Try from newest to oldest
                            for dbtn in reversed(dbtns):
                                try:
                                    ...
                                    if len(b64_data) > 10000:
                                        log.info(f"UI Auto: Successfully downloaded video via button ({len(b64_data)} bytes).")
                                        if "download_finished" not in perf_timestamps: perf_timestamps["download_finished"] = perf_time.perf_counter()
                                        existing_srcs.append(src)
                                        media_list.append({
                                            "image": {
                                                "generatedImage": {
                                                    "url": f"data:video/mp4;base64,{b64_data}",
                                                    "name": f"flow_video_{int(_time.time())}_{len(media_list)}",
                                                }
                                            }
                                        })
                                        if callback:
                                            try:
                                                await callback(f"data:video/mp4;base64,{b64_data}")
                                            except Exception:
                                                pass
                                        downloaded = True
                                        break
                                except Exception as e:
                                    log.warning(f"UI Auto: Download button click failed: {e}")
                        except Exception as e:
                            log.warning(f"UI Auto: Error searching for download buttons: {e}")
                    ...
                    if not downloaded:
                        ...
                        if not src.startswith('blob:'):
                            ...
                            # Do NOT add blob: to existing_srcs if we failed, allow it to retry in the next iteration
                            await asyncio.sleep(1)
