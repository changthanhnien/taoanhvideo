...
class TeleaEngine:
    ...
    def process_image(self, input_path: Path, output_path: Path, rect: tuple, params: dict):
        ...
        radius = max(1, min(int(params.get("radius", 5)), 25))
        out_bgr = cv2.inpaint(img_bgr, roi_mask, radius, cv2.INPAINT_TELEA)
        ...
        try:
            ...
            while True:
                ...
                if progress_cb and idx % 5 == 0:
                    progress_cb(idx, total_frames, "Đang xóa watermark bằng Telea...")
                    
                out_bgr = cv2.inpaint(frame, roi_mask, radius, cv2.INPAINT_TELEA)
