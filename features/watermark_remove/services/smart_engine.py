...
from features.watermark_remove.services.smart_remover import SmartWatermarkRemover

class SmartEngine:
    def __init__(self, sensitivity='medium'):
        self.remover = SmartWatermarkRemover(sensitivity=sensitivity)
    ...
    def process_image(self, input_path: Path, output_path: Path, rect: tuple, params: dict):
        ...
        stats = self.remover.analyze_image(
            img_bgr, 
            [x, y, x+rw, y+rh], 
            roi_mask=roi_mask,
            gain=params.get("gain"),
            floor=params.get("floor"),
            edge_expand=params.get("edge_expand"),
            tophat_thr=params.get("tophat_thr"),
            despill=params.get("despill"),
            edge_blur=params.get("edge_blur")
        )
        
        out_bgr = self.remover.process_frame(img_bgr)
    ...
    def process_video(self, input_path: Path, output_path: Path, rect: tuple, params: dict, progress_cb=None):
        ...
        try:
            ...
            if progress_cb:
                progress_cb(0, 0, "Đang phân tích temporal median...")
            ...
            while True:
                ...
                if progress_cb and idx % 5 == 0:
                    progress_cb(idx, total_frames, "Đang xóa watermark...")
                
                out_bgr = self.remover.process_frame(frame)
